...
function AutoSizeVirtualList<T>({
  ...
  function autoSizeVirtualList({
    ...
    if (listEl.current && itemList.length) {
      ...
      const endIndex = Math.min(itemList.length, startIndex + viewCount + offset)
      const renderItems = []
      ...
      if (bottom) {
        const { clientHeight: curHeight } = containerEl.current as HTMLDivElement
  ...
  return (
    <div className='auto-virtual'>
      ...
      >
        ...
        <div className='auto-virtual-list_container' ref={innerEl} style={{ ...style }}>
          {renderedItem}
        </div>
      </div>
      <div
      ...
      >
        <div ref={barEl} className='auto-virtual-bar' onMouseDown={handleMouseDown}></div>
      </div>
    </div>
  )
}
